...
@section('title', 'Verifikasi Penyelia Sertifikat')
@section('content')
    <div class="page-heading">
        <h3>Verifikasi Penyelia Sertifikat</h3>
    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    ...
                    <div class="col-sm-4">
                        <input class="form-control" id="tanggalVerifPenyelia" name="tanggalVerifPenyelia" type="date">
                    </div>
                    <label class="col-sm-2 col-form-label">Tanggal Ver. Admin</label>
                    <div class="col-sm-4">
                        <input class="form-control readonlyInput" id="tanggalVerifAdmin"
                            name="tanggalVerifAdmin" type="text" readonly>
                    </div>
                </div>
                <div class="row mb-3">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#sertifikat').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('verifikasi_penyelia.sertifikat.load') }}",
                    type: "POST",
                    data: function(data) {
        ...
        function verif(nomorSurat) {
            $.ajax({
                url: '{{ route("verifikasi_penyelia.sertifikat.verif") }}',
                type: 'POST',
                data: {
        ...
        $('#VerifPenyelia').on('click', function() {
            ...
            if (!tanggalVerifPenyelia) {
                ...
                if (tanggalVerifPenyelia < tanggalPinjam) {
                    Swal.fire({
                        title: "Peringatan!",
                ...
                if (tanggalVerifPenyelia < tanggalVerifAdmin) {
                    Swal.fire({
                        title: "Peringatan!",
                        text: "Tanggal verifikasi tidak boleh lebih kecil dari tanggal Verif Admin!",
                        icon: "warning"
                    });
                    return;
                }
            $.ajax({
                url: '{{ route("verifikasi_penyelia.sertifikat.verifikasi_penyelia") }}',
                type: 'POST',
                data: {
                    nomorSurat: nomorSurat,
                    tanggalVerifPenyelia: tanggalVerifPenyelia,
                    _token: '{{ csrf_token() }}'
                },
                success: function(data) {
                    $('#VerifPenyelia').addClass('d-none');
        ...
        $('#TolakVerif').on('click', function() {
            ...
            $.ajax({
                url: '{{ route("verifikasi_penyelia.sertifikat.tolak") }}',
                type: 'POST',
                data: {
                    nomorSurat: nomorSurat,
                    kodeSkpd: kodeSkpd,
        ...
        $('#BatalkanVerif').on('click', function() {
            ...
            $.ajax({
                url: '{{ route("verifikasi_penyelia.sertifikat.batalkan") }}',
                type: 'POST',
                data: {
                    nomorSurat: nomorSurat,
                    _token: '{{ csrf_token() }}'
