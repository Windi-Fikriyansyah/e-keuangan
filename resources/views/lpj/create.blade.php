...
@section('content')
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('lpj.store') }}" id="formBpkb">
                    @csrf
                    ...
                    <div class="table-responsive">
                        ...
                        <form id="filterForm">
                            <div class="row mb-3 align-items-center">
                                <div class="col-sm-3">
                                    <input class="form-control" type="date" name="tgl_awal" id="tgl_awal" value="{{ old('tgl_awal') }}">
                                </div>
                                <div class="col-sm-3">
                                    <input class="form-control @error('tgl_akhir') is-invalid @enderror" type="date" name="tgl_akhir" id="tgl_akhir" value="{{ old('tgl_akhir') }}">
                                    @error('tgl_akhir')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <!-- Tambahkan margin-bottom agar tabel tidak terlalu dekat -->
...
document.getElementById("formBpkb").addEventListener("submit", function(e) {
    ...
    $(document).ready(function() {
        // Handle click event for Tampilkan button
        ...
        function loadData() {
            ...
            $('#tabelPotonganBody').html('<tr><td colspan="7" class="text-center">Loading data...</td></tr>');

            // Fetch data from server
            $.ajax({
                url: '{{ route("lpj.getData") }}',
                type: 'GET',
                data: {
                    tgl_awal: tgl_awal,
                    tgl_akhir: tgl_akhir
    ...
    return 'Rp ' + rupiah;
                        ...
                        $('#tabelPotonganBody').html('<tr><td colspan="8" class="text-center">Tidak ada data</td></tr>');
                ...
                error: function(xhr, status, error) {
                    console.error(error);
                    $('#tabelPotonganBody').html('<tr><td colspan="8" class="text-center">Error loading data</td></tr>');
        ...
        function setupDeleteButtons() {
            $('.btn-hapus').on('click', function() {
                var id = $(this).data('no_bukti');
                var noBukti = $(this).data('no-bukti');

                Swal.fire({
                    title: 'Konfirmasi',
                    text: `Apakah Anda yakin ingin menghapus transaksi dengan No Bukti ${noBukti}?`,
                    icon: 'warning',
