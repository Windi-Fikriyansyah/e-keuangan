    ...
    <script>
        $(document).ready(function() {
            ...
            let bpkb = $('#bpkb').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('verifikasi_admin.bpkb.load') }}",
                    type: "POST",
                    data: function(data) {
                ...
                createdRow: function(row, data, index) {
                    if (data.statusVerifAdmin == "1" && data.statusVerifPenyelia != "1" && data.statusTolak !== "1") {
            ...
            $('#bpkb tbody').on('click', '.verifikasi', function() {
                ...
                $('#tanggalVerifikasi').val(selectedData.tanggalVerifAdmin);
                $('#nomorSurat').val(selectedData.nomorSurat);
                $('#tanggalPinjam').val(tanggalIndonesia(selectedData.tanggalPinjam));
                $('#tanggalVerifikasiOperator').val(tanggalIndonesia(selectedData
                    .tanggalVerifikasiOperator));
                ...
                else if (selectedData.statusVerifAdmin != '1') {
                    $('#setujuVerifikasi').prop('hidden', false);
                    $('#TolakVerif').prop('hidden', false);
                } else if (selectedData.statusVerifAdmin == '1' && selectedData.statusVerifPenyelia !=
                    '1') {
            ...
            $('#setujuVerifikasi').on('click', function() {
                ...
                if (tanggalVerifikasi < selectedData.tanggalVerifikasiOperator) {
                    Swal.fire({
                        title: "Peringatan!",
                        text: "Tanggal verifikasi tidak boleh lebih kecil dari tanggal verifikasi operator!",
                        icon: "warning"
                    });
                    return;
                }
                ...
                $.ajax({
                    url: "{{ route('verifikasi_admin.bpkb.verifikasi') }}",
                    type: "POST",
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function(response) {
                        ...
                        $('#tanggalVerifikasiOperator').val(null);
            ...
            $.ajax({
                url: '{{ route("verifikasi_admin.bpkb.tolak") }}',
                type: 'POST',
                data: {
